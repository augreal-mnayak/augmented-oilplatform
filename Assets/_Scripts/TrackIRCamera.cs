...
public class TrackIRCamera : MonoBehaviour {
    ...
    private Vector3Serializer currentPosition;

    private string ipAddr;

   // Create UDP client
   int receiverPort = 20016;

	public TrackIRCamera ()
	{
		//Debug.Log ("Constructor TrackIRCamera");
...
//
    ...
    {
        Debug.Log("Button click event handler");
        IPAddress devMachineIPAddr = IPAddress.Parse("192.168.243.130");
        IPEndPoint devMachineEndPoint = new IPEndPoint(devMachineIPAddr, receiverPort);

        var formatter = new BinaryFormatter();

        //send serialized rotation data
        using (MemoryStream memoryStream = new MemoryStream())
        {
            formatter.Serialize(memoryStream, "RESET");

            if (receiver != null)
            {
                try
                {
                    receiver.Send(memoryStream.ToArray(), (int)memoryStream.Length, devMachineEndPoint);
                    Debug.Log("Command sent");
                }
                catch (Exception ex)
                {
                    Debug.Log("Error sending command: " + ex.Message);
                }
            }
            else
            {
    ...
    void Update()
    {
        ...
        if (running)
        {
            ...
            //TrackIRClient.LPTRACKIRDATA tid = trackIRclient.client_HandleTrackIRData(); // Data for head tracking
            //Vector3 pos = trackIRCamera.transform.localPosition;                          // Updates main camera, change to whatever
            Vector3 pos = currentPosition.V3;
            ...
            if (Camera.main != null)
            {
                ...
                //Debug.Log("After update camera rotation: " + Camera.main.transform.localRotation);
                //Debug.Log("After update camera rotation: " + currentRotation);
                //Debug.Log("After update camera position: " + Camera.main.transform.localPosition);
            }
        }
    }
    ...
    private void DataReceived(IAsyncResult ar)
    {
        ...
        HeadTracker trackedPos = (HeadTracker)obj;
        //Vector3Serializer headrot = (Vector3Serializer)obj;
        //currentRotation = headrot;
        currentPosition = trackedPos.Position;
        currentPosition.z -= 200;
        currentRotation = trackedPos.Rotation;


        //Debug.Log("Deserialized rotation data: x - " + currentRotation.x + "y - " + currentRotation.y + "z- " + currentRotation.z);
        //Debug.Log("Deserialized camera rotation: " + currentRotation);
        //Debug.Log("Deserialized camera position: " + currentPosition);
